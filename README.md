...
**Stack at a glance:** React + Vite + Tailwind SPA (Vercel) → FastAPI + SQLAlchemy REST API (AWS ECS Fargate) → PostgreSQL (Neon). JWT auth, 60 endpoints, 16 data models, 1255 automated tests (510 pytest + 745 Vitest).
...
**pytest** - backend tests (510) for the scaling service and its folk-unit vocabulary, and the authorization surface (visibility, sharing/grants, blocking, the invite-token flow, the invite link-preview card, the source/cuisine autosuggest scope, the friend graph, signup + account-edit validation).

**Vitest + React Testing Library** - frontend unit/component tests (745 in 51 files: quantity parsing, imprecise-measure labelling, handoff/invite flows, form and page components, plus design-token invariants). Run with `npm test` in `frontend/`.
...
| POST | /friends/reports | Yes | **Report a person for review** (#87) — the other half of blocking, and a different act: a block is something you do FOR YOURSELF and works instantly, a report goes to whoever runs the app and works when a human looks. Only having the first leaves someone free to move on to the next person. Body is `{user_id, reason, note?}`; `reason` is one of `harassment` \| `spam` \| `inappropriate` \| `impersonation` \| `other`, and the note is the reporter's own words (optional, 1000 chars) — a reason alone is a valid report, because demanding an explanation is friction in front of someone who may be upset. **Deliberately NOT gated on blocking in either direction**, unlike every other people-returning route here: otherwise someone harasses you, blocks you, and becomes permanently unreportable. The reported person is **never told** — telling them turns a safety mechanism into an escalation trigger. One OPEN report per reporter per person: a second tap while the first is open is accepted and discarded, so the table can't be flooded and a moderator sees one row per grievance; a report filed after the first was CLOSED is a new report, because it means it happened again. 204 either way, so the caller can't tell a duplicate from a first ("you already reported them" makes someone doubt the first one landed). 400 on yourself, 404 on an unknown user. There is **no endpoint to read reports back** — that needs an admin role this app has no concept of. |
...
*59 application routes on this branch — the table is the whole product surface. Counts have changed several times as features were added and removed, so verify rather than trust: `grep -rn "^@router\.\|^@app\." app/` (router decorators + `GET /health`, declared on the app itself in `app/main.py`).*
