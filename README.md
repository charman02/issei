...
**Stack at a glance:** React + Vite + Tailwind SPA (Vercel) → FastAPI + SQLAlchemy REST API (AWS ECS Fargate) → PostgreSQL (Neon). JWT auth, 59 endpoints, 15 data models, 1193 automated tests (472 pytest + 721 Vitest).
...
**pytest** - backend tests (472) for the scaling service and its folk-unit vocabulary, and the authorization surface (visibility, sharing/grants, blocking, the invite-token flow, the invite link-preview card, the source/cuisine autosuggest scope, the friend graph, signup + account-edit validation).

**Vitest + React Testing Library** - frontend unit/component tests (721 in 50 files: quantity parsing, imprecise-measure labelling, handoff/invite flows, form and page components, plus design-token invariants). Run with `npm test` in `frontend/`.
...
| PATCH | /posts/{post_id} | Yes | **Edit your own meal** — dish name, the line under it, who can see it, and optionally the attached recipe. Author-only; a non-author gets **404**, not 403, the same answer DELETE gives, so neither confirms the post exists to someone not entitled. Partial: `null` means "leave it alone", so a description is cleared by sending `""`, and `recipe_id: 0` detaches (0 isn't a valid id, so it reads unambiguously). Attaching still requires a recipe you OWN. It does NOT change the author (a post is "I made this"), does NOT touch the read-mark (`is_new` keys on the post id, which an edit doesn't move, so an edit can't resurface as unread), and notifies nobody. |
