...
**Stack at a glance:** React + Vite + Tailwind SPA (Vercel) → FastAPI + SQLAlchemy REST API (AWS ECS Fargate) → PostgreSQL (Neon). JWT auth, 60 endpoints, 16 data models, 1269 automated tests (519 pytest + 750 Vitest).
...
**pytest** - backend tests (519) for the scaling service and its folk-unit vocabulary, and the authorization surface (visibility, sharing/grants, blocking, the invite-token flow, the invite link-preview card, the source/cuisine autosuggest scope, the friend graph, signup + account-edit validation).

**Vitest + React Testing Library** - frontend unit/component tests (750 in 51 files: quantity parsing, imprecise-measure labelling, handoff/invite flows, form and page components, plus design-token invariants). Run with `npm test` in `frontend/`.
...
| POST | /recipes/{recipe_id}/save | Yes | **Keep** a recipe you didn't write — a bookmark, not a copy. Gated on `can_view`, so you can only keep what you can already read and keeping can never widen access; 404 otherwise, 400 on your own recipe. Idempotent (UNIQUE(user, recipe)). A NEW keep notifies the cook — **anonymously** (#96): the row stores the keeper's id because `notify()` needs it, but every actor field is nulled on the way out, so the line reads "Someone kept your Adobo". Deduped while unread, so a keep/unkeep loop can't flood the inbox. `GET /recipes/{id}` also gains `keeper_count`, populated **only for the owner** (`None`, never `0`, for anyone else) — the cook learns how many, never who, and there is no keeper list anywhere. Not on THIS endpoint's response: the owner is rejected with a 400 above, so the caller here is never the owner. |
...
| POST | /friends/reports | Yes | **Report a person** (#87) — the other half of blocking, and a different act: a block is something you do FOR YOURSELF and works instantly, a report goes to whoever runs the app and works when a human looks. Only having the first leaves someone free to move on to the next person. Body is `{user_id, reason, note?}`; `reason` is one of `harassment` \| `spam` \| `inappropriate` \| `impersonation` \| `other`, and the note is the reporter's own words (optional, 1000 chars) — a reason alone is a valid report, because demanding an explanation is friction in front of someone who may be upset. **Deliberately NOT gated on blocking in either direction**, unlike every other people-returning route here: otherwise someone harasses you, blocks you, and becomes permanently unreportable. The reported person is **never told** — telling them turns a safety mechanism into an escalation trigger. One OPEN report per reporter per person: a second tap while the first is open is accepted and discarded, so the table can't be flooded and whoever reads them sees one case per grievance rather than a thread (a second report with new words APPENDS to the open row — the first version discarded it, which meant the app said "we'll take a look" about something it had thrown away); a report filed after the first was CLOSED is a new report, because it means it happened again. 204 either way, so the caller can't tell a duplicate from a first ("you already reported them" makes someone doubt the first one landed). 400 on yourself, 404 on an unknown user. There is **no endpoint to read reports back** — that needs an admin role this app has no concept of. |
...
| GET | /posts/browse | Yes | Every post with `visibility == "public"`, newest first, **uncapped** (no `?q=` — filtering was always client-side, which is why nothing limits the response). Unlike the everyone-feed scope it includes the caller's own and friends' public posts. Same recipe-link nulling as the feed. **No client calls this today**: it was built for Browse's Meals tab, which #94 un-shipped once it was clear Browse is an intent surface and nobody searches for a photo of someone's dinner. Kept because the data is right and a "most asked for" row (#82) will want it; treat it as unused surface, not a feature. |
| GET | /posts/{post_id} | Yes | A single post — visible to its author, a friend of the author, or **anyone** on a public post (which is what makes a public meal openable by someone who isn't a friend — it was built for Browse's Meals tab, and outlives it: #94 removed that tab, and the rule still matters for a link someone sends); a viewer not entitled gets 404 (don't confirm it exists). Same recipe-link nulling as the feed. |
| PATCH | /posts/{post_id} | Yes | **Edit your own meal** — exactly three fields: dish name, the line under it, and who can see it. Author-only; a non-author gets **404**, not 403, the same answer DELETE gives, so neither confirms the post exists to someone not entitled. Partial: `null` means "leave it alone", so a description is cleared by sending `""`. Four things it deliberately does NOT do. It doesn't change the **author** (a post is "I made this"). It doesn't touch the **photo** — a different photo is a different meal, so that's a new post, and the field had none of the Cloudinary-host validation `PATCH /auth/me` applies. It doesn't touch the **attached recipe** — attaching one to a post people asked about is *answering* them, which is `POST /{id}/fulfill`'s whole job (grants + notifications included); doing it quietly here would attach the recipe and leave every ask pending underneath it. And it doesn't touch the **read-mark** or notify anyone (`is_new` keys on the post id, which an edit doesn't move, so an edit can't resurface as unread). One consequence worth knowing: lowering visibility hides the post from a non-friend who asked, so `DELETE /{id}/request` deliberately does **not** require read access — see its row. |
...
| DELETE | /posts/{post_id}/request | Yes | Take back your ask. Removes only the caller's own **pending** row — a fulfilled request is the record that a recipe was handed over. The cook's notification is deliberately kept: they were told something true. **The only post route that does not require read access**, and deliberately: ask on a public meal, watch the cook set it to friends-only, and a `can_view_post` gate here would make your own ask permanent — 404 on the post *and* 404 on withdrawing it, with your name still on the cook's `/requests` forever. A row you created is yours to delete; it isn't a read of someone else's content. It still needs a **pending row or** read access, though — without that, polling this route would read a private post's dish name, photo and author out of the response body. And when the pending row is the ONLY credential it answers **204 with no body**, not 200: since `PATCH /posts/{id}` lets a cook rename the dish and rewrite the description in the same call that hides it, the body would be content that was never on the asker's screen. Nothing needs it — both clients read exactly one field (`requested_by_me`) off this response, and after a successful retract that field is false, which is what an absent body already means to them. |
...
| GET | /notifications | Yes | The caller's inbox — issei's first notification surface. Newest first, keyset-paginated on `?before_id=` (page 30), returning `{notifications, unread_count}` so the badge can't drift from the rows. Scoped to `user_id == caller`. Five types: `recipe_request`, `request_fulfilled`, `friend_request`, `friend_accept`, `recipe_kept` — the last **anonymous at the API boundary** (#96): every actor field is nulled on the way out, so the cook learns how many people kept a recipe, never who — friend events were retrofitted onto the same inbox rather than getting their own counter. A row whose post or recipe was deleted (FK `SET NULL`) comes back without a link; the line still reads. |
...
*60 application routes on this branch — the table is the whole product surface. Counts have changed several times as features were added and removed, so verify rather than trust: `grep -rn "^@router\.\|^@app\." app/` (router decorators + `GET /health`, declared on the app itself in `app/main.py`).*
