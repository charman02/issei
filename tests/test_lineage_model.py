...
def test_recipe_has_lineage_defaults(db_session):
    ...
    u = User(first_name="A", last_name="B", email="a@b.com", hashed_password=hash_password("x"))
    db_session.add(u)
    db_session.commit()
    ...
    child = Recipe(
        user_id=u.id, name="Congee (mine)", parent_recipe_id=root.id, lineage_relation="remixed"
    )
...
def test_ghost_cook_handoff_models(db_session):
    ...
    u = User(first_name="A", last_name="B", email="g@b.com", hashed_password=hash_password("x"))
    db_session.add(u)
    db_session.commit()
    ...
    ghost = GhostAncestor(
        recipe_id=r.id, name="Lola", place="Cebu", year="1960s", memory="Made it every Sunday"
    )
    ...
    db_session.add_all([ghost, cook, pass_])
    db_session.commit()
