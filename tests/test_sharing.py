...
def test_can_view_owner_public_and_grant(db_session, make_user):
    ...
    db_session.add(
        Handoff(recipe_id=root.id, from_user_id=owner.id, to_user_id=other.id, state="accepted")
    )
    db_session.commit()
    ...
    other2, _ = make_user()
    db_session.add(
        Handoff(recipe_id=root.id, from_user_id=owner.id, to_user_id=other2.id, state="pending")
    )
    db_session.commit()
...
def test_handoff_email_is_pending(client, make_user, db_session):
    ...
    root = client.post("/recipes", json=_payload(), headers=oheaders).json()
    client.post(
        f"/recipes/{root['id']}/handoff", json={"to_email": "mom@example.com"}, headers=oheaders
    )
...
def test_email_invite_auto_accepts_on_signup(client, make_user, db_session):
    ...
    root = client.post("/recipes", json=_payload(), headers=oheaders).json()
    client.post(
        f"/recipes/{root['id']}/handoff", json={"to_email": "newmom@example.com"}, headers=oheaders
    )
    ...
    h = (
        ...
        .filter_by(recipe_id=root["id"], to_email="newmom@example.com")
        .one()
    )
    assert h.state == "accepted"
...
def test_shared_with_me_lists_accepted_only(client, make_user):
    ...
    assert "My Own" not in [
        r["name"] for r in client.get("/recipes/shared", headers=gheaders).json()
    ]
...
def test_scale_gated_by_can_view(client, make_user):
    ...
    assert (
        client.get(f"/recipes/{root['id']}/scale?servings=2", headers=sheaders).status_code == 404
    )
    assert (
        client.get(f"/recipes/{root['id']}/scale?servings=2", headers=oheaders).status_code == 200
    )
...
def test_browse_hides_shared_with_count(client, make_user):
    ...
    owner, oheaders = make_user()
    grantee, _ = make_user()
    root = client.post("/recipes", json=_payload(visibility="public"), headers=oheaders).json()
    client.post(
        f"/recipes/{root['id']}/handoff", json={"to_user_id": grantee.id}, headers=oheaders
    )  # accepted grant
