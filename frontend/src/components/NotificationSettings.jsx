...
//   is in right now.
//
// THERE IS NO SWITCH FOR `notify_people`, and its absence is deliberate rather than an oversight.
// Person-to-person pushes (someone asked for your recipe, someone accepted your friend request)
// are not wired to the sender yet — `notify()` writes the inbox row and nothing pushes it. A
// switch for that would be a control that changes nothing, which is worse than a missing one:
// switching it off would read as a promise the app then breaks in the other direction. It ships
// the day the push is wired. See TECHDEBT.
...
export default function NotificationSettings() {
  ...
  async function toggleDevice(next) {
    ...
    if (result.ok) {
      ...
      if (next && user.notify_prompt === false) await savePref({ notify_prompt: true })
    } else {
  ...
  const promptOn = user.notify_prompt !== false
  ...
  // person can find out.
  const selfCancelling = promptOn && inQuietHours(hour, quietFrom, quietTo)
    ...
    <>
      ...
      <div className="sticker bg-card px-5 py-2">
        ...
        <div className="border-t-2 border-line">
          <Toggle
            label="A daily nudge to share a meal"
            hint="Once a day, at the time below, if your friends have been cooking."
            on={promptOn}
            disabled={savingPref}
            onChange={(v) => savePref({ notify_prompt: v })}
          />
        </div>
          ...
          <>
            ...
            <div className="border-t-2 border-line py-2.5">
              ...
              <p className="font-display italic text-[12px] text-ink-soft mt-1 leading-snug">
                {quietOff
                  ? 'Set to the same hour, so there are no quiet hours — a nudge can arrive whenever it’s due.'
                  : 'Nothing arrives inside these hours, even a nudge that ran late.'}
              </p>
            </div>
          </>
        )}
