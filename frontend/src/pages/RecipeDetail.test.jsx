...
vi.mock('../api/lineage', () => ({
  cookRecipe: vi.fn(() => Promise.resolve({ data: { cook_count: 4 } })),
}))
beforeEach(() => {
...
describe('RecipeDetail cook action', () => {
  ...
  it('does not offer remix ("make it mine") — remix is cut from v1', async () => {
    renderDetail()
    expect(
      await screen.findByRole('button', { name: /i cooked this/i }),
    ).toBeInTheDocument()
    expect(
      screen.queryByRole('button', { name: /make it mine/i }),
...
describe('RecipeDetail living-recipe weavings', () => {
  it('shows a woven voice-note quote under its step', async () => {
        ...
        steps: [
          {
            id: 1,
            position: 0,
            content: 'Simmer the pork.',
            voice_note: 'until it smells like Sunday',
          },
        ],
        cook_count: 3,
        child_count: 0,
    ...
    renderDetail()
    expect(
      await screen.findByText(/until it smells like Sunday/i),
    ).toBeInTheDocument()
  })
