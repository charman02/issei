...
vi.mock('../api/lineage', () => ({
  remixRecipe: vi.fn(() => Promise.resolve({ data: { id: 99 } })),
}))
      ...
      <button
        onClick={() =>
          onSubmit({
            name: 'My Adobo',
            notes: 'less salt',
            ingredients: [{ name: 'lard', position: 1 }],
            steps: [],
          })
        }
      >
  ...
  it('seeds notes from parent, forwards edited scalars, and remixes with the prompt answer', async () => {
    render(
      <MemoryRouter initialEntries={['/recipes/1/remix']}>
        <Routes>
          <Route path="/recipes/:id/remix" element={<RemixRecipe />} />
          <Route path="/recipes/:id" element={<div>child page</div>} />
        </Routes>
      </MemoryRouter>,
    )
    ...
    await userEvent.type(
      screen.getByPlaceholderText(/what makes yours yours/i),
      'Mom used lard',
    )
