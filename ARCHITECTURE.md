...
| `UserProfile.jsx` | `/u/:userId` | Another person's read-only profile — name, photo, their `profile_visibility`, the caller-side friend button (add / accept / requested / remove), and their recipe + post counts, each already gated by `can_view` / `can_view_post` server-side. Content grid via `ProfileContent`. At the **bottom of the page, below the recipes and posts**, sits a small brick **"Block"** chip (#85) — the person's name appears in the CONFIRM ("Block Ana?"), not on the resting control. It used to sit directly under the friend button, which put a safety control inside the social one's blast radius: the two most consequential taps on the page were adjacent. Down here it is still the last thing on the page rather than hidden, so you reach it by deciding to, having scrolled past everything the person actually cooks. Hidden on your own profile, and kept secondary to the friend action by being a chip rather than by being faint: it's a safety control people need to FIND, and the grey underlined link it started as read as a footnote — whose two-tap confirm names every consequence (mutual invisibility, the deleted friendship, and that a recipe you already sent them stays theirs) rather than asking "are you sure?" about nothing. On success it navigates to `/friends`, because this page is now a 404 for the caller. |
...
- **Verifying changes:** backend has `pytest` (**510 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**745 tests in 51 files**) — run
