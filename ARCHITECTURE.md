...
| `api/posts.js` | The presence-feed calls (`createPost`, `getFeed` — takes an optional `scope`, `browsePosts` — public posts for Browse, `getPost`, `deletePost`, `getUserPosts` — the `/posts` routes), built on `client`. Powers `Feed`, `Browse`'s Meals tab, `PostComposer`, `PostPage`, and the profile post grid. (`deletePost` had shipped in the API layer long before anything called it — there was no way to delete a post from the UI at all until the control landed on `PostPage`.) |
...
- **Verifying changes:** backend has `pytest` (**472 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**721 tests in 50 files**) — run
