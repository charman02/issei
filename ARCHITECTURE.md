...
| `UserProfile.jsx` | `/u/:userId` | Another person's read-only profile — name, photo, their `profile_visibility`, the caller-side friend button (add / accept / requested / remove), and their recipe + post counts, each already gated by `can_view` / `can_view_post` server-side. Content grid via `ProfileContent`. Below the friend action sits a **"Block {name}"** button in brick (#85) — hidden on your own profile, and kept secondary to the friend action by being a compact pill rather than by being faint: it's a safety control people need to FIND, and the grey underlined link it started as read as a footnote — whose two-tap confirm names every consequence (mutual invisibility, the deleted friendship, and that a recipe you already sent them stays theirs) rather than asking "are you sure?" about nothing. On success it navigates to `/friends`, because this page is now a 404 for the caller. |
...
- **Verifying changes:** backend has `pytest` (**436 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**693 tests in 50 files**) — run
