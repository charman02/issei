...
| `api/friends.js` | The friend-graph calls (`requestFriend`, `acceptFriend`, `removeFriend`, `getFriends` — takes an optional `order`, `getFriendRequests`, `getFriendSuggestions`, `discoverPeople` — the #80 app-wide directory with an optional name search, `getUserProfile`, plus `blockUser` / `unblockUser` / `getBlocks` — #85), built on `client`. Powers `Friends`, `UserProfile`, `FriendsStrip`, `Profile`, and the You page's counts. |
...
- **Verifying changes:** backend has `pytest` (**473 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**730 tests in 51 files**) — run
